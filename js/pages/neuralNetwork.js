...
export function renderNeuralNetwork(container) {
  ...
  container.innerHTML = `
    ...
    <div class="page-header">
      ...
      <span class="lesson-number">Lesson 4 — Deep Learning</span>
...
model.<span class="fn">fit</span>(X, y, epochs=<span class="num">100</span>)</pre>
    ...
    <div class="steps-section">
      ...
      <div class="steps-header">
        ...
        <h3>Neural Networks — Step by Step</h3>

        <p>Understanding forward pass, backpropagation, and how weights update through layers</p>
      ...
      <div class="steps-grid">
        ...
        <div class="step-card">
          ...
          <div class="step-body">The network is trained on labeled examples. <strong>Training data</strong> teaches the weights. <strong>Test data</strong> measures generalization — can the network handle inputs it has never seen before?</div>

          <div class="step-formula">80% Train → learn weights | 20% Test → measure accuracy</div>
        ...
        <div class="step-card">
          ...
          <div class="step-body">Every connection between neurons starts with a small <strong>random weight</strong>. If weights started at zero, all neurons would learn the same thing. Random initialization <em>breaks symmetry</em> so each neuron can specialize.</div>

          <div class="step-formula">w ~ N(0, σ²)  — e.g. Xavier / He initialization</div>
        ...
        <div class="step-card">
          ...
          <div class="step-title">Forward Pass (Predict)</div>

          <div class="step-body">Input data flows <strong>forward</strong> through each layer. Each neuron computes a weighted sum of its inputs, adds a bias, and passes the result through an <strong>activation function</strong> (ReLU), which adds non-linearity.</div>

          <div class="step-formula">a = ReLU(W·x + b)</div>
        ...
        <div class="step-card">
          ...
          <div class="step-body">The output is compared to the correct label. <strong>Cross-entropy loss</strong> quantifies how wrong the prediction was. Unlike MSE, cross-entropy works well for classification because it heavily penalizes confident wrong predictions.</div>

          <div class="step-formula">Loss = −Σ yᵢ·log(ŷᵢ)</div>
        ...
        <div class="step-card">
          ...
          <div class="step-title">Backpropagation (Update Weights)</div>

          <div class="step-body">The loss is sent <strong>backward</strong> through the network using the chain rule of calculus. Each weight learns how much it contributed to the error and gets nudged in the direction that reduces it. The learning rate scales the nudge.</div>

          <div class="step-formula">∂Loss/∂w via chain rule → w ← w − α · ∂Loss/∂w</div>
        ...
        <div class="step-card">
          ...
          <div class="step-body">Forward pass → Loss → Backprop repeats every <strong>epoch</strong>. Over hundreds of epochs the decision boundary becomes more accurate. Watch how the colorful regions on the canvas sharpen as training progresses!</div>

          <div class="step-formula">Repeat: Forward → Loss → Backward → Update</div>
  ...
  function trainLoop() {
    ...
    for (let i = 0; i < 3; i++) trainStep(); // batch steps for speed
